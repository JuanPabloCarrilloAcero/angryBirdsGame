const { Engine, World, Body, Bodies, Constraint, Composite, Composites } =
  Matter;
...
function setup() {
  ...
  ground = createRect(width / 2, height - 10, width, 20, "ground", {
    isStatic: true,
  });
...
function drawSlingshot() {
  if (slingshot.bodyB) {
    image(
      slingshotTexture,
      slingshot.pointA.x - 40,
      slingshot.pointA.y - 29,
      80,
      160
    );
    stroke(37, 17, 12);
    ...
    line(
      slingshot.pointA.x,
      slingshot.pointA.y,
      slingshot.bodyB.position.x,
      slingshot.bodyB.position.y
    );
  }
}
...
function draw() {
  ...
  if (birdLaunched) {
    ...
    if (
      Math.abs(cameraX - targetCameraX) < 1 &&
      Math.abs(cameraScale - 1) < 0.01
    ) {
